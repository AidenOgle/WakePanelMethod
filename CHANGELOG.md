...
- Created module `initialization.py` to contain `generateNACAfoil(...)`, `body_creation(...)`, `pitching(...)`, and `vortexblobinit(...)`. Potentially could add a function similar to `pitching(...)` for flow velocity initialization
